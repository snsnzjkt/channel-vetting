...
actually delete, and note the two guards that still apply on top of it:
  ...
  - The mass-delete circuit breaker (see MAX_DELETES / MAX_DELETE_FRACTION),
    modelled on scripts/backfill/cleanup_external_duplicates.py: refusing an unexpectedly large
...
core/http_client.py. Don't add a bare requests call here.
"""
import argparse
import logging
import sys
...
# pipeline.location_drop_reason, and reimplementing any part of it here is exactly
...
# Circuit breaker, same shape as scripts/backfill/cleanup_external_duplicates.py. Both bounds
