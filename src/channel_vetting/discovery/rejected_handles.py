...
pipeline.TRANSIENT_DROP_REASONS.
...
from channel_vetting.config import (
    ...
    REJECTED_HANDLES_RETENTION_DAYS,
)
...
def record(niche_name: str, handles) -> int:
    ...
    # UNIQUE PER PROCESS. This was f"{REJECTED_HANDLES_FILE}.tmp" — one shared name —
    ...
    tmp_path = f"{REJECTED_HANDLES_FILE}.{os.getpid()}.tmp"
    try:
