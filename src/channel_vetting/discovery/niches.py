...
Extracted from `pipeline.py` (2026-08-14) so `outreach/sender.py` can read the niche ->
...
`pipeline.py` re-exports NICHES from here, so this stays the single source of
...
NICHES = {
    "Home Theater": {
        ...
        # RESCUE vocabulary for pipeline.off_target_reason — NEVER an admission
        ...
        #
        # These terms only ever RESCUE — see pipeline.off_target_reason. Adding one
        ...
        # See verification.gemini.GeminiVerifier.judge for why that asymmetry is the
        ...
        "video_criteria": [
            ...
            # videos "Range Day 47" matches nothing, and verification/video_topics.py closes
            ...
            # not obtainable — see verification/video_topics.py for the measured 200/empty-body
        ...
        # scripts/analysis/measure_discovery_pool.py probes, and flipping this back to
...
# ---------------------------------------------------------------------------
# Everything below moved here from pipeline.py (2026-08-14) because
# wire_discovery_filters() MUTATES the NICHES dict above, in place, at import.
# While it lived in pipeline.py, `import niches` on its own returned a registry
...
#
# pipeline.py imports EXCLUDED_TOPIC_TERMS back from here for its local
...
# YouTube Data API v3 (see discovery.influencers_club._to_candidate).
#
...
def wire_discovery_filters(niches: dict) -> None:
    ...
    for niche_config in niches.values():
        ...
        # each; neither is the relevance decision, which pipeline.off_target_reason
