...
the base (see enrichment/external_dedupe.py), and removes them from OUR two
...
Usage:
    python scripts/backfill/cleanup_external_duplicates.py              # dry run
    python scripts/backfill/cleanup_external_duplicates.py --confirm     # actually delete
    python scripts/backfill/cleanup_external_duplicates.py --confirm --yes-delete-many
                                                       # ... even if the
                                                       # match set is huge

SAFETY MODEL — this is the only script in the repo that PERMANENTLY
deletes Airtable rows, and there is no undo on our end
(airtable.client.delete_record). Three independent guards, in the order
they fire:
...
1. `--confirm` is required. Without it `main()` returns before the delete
   ...
   same shape of gate as scripts/audit/audit_blocklist.py's `--mark`.
...
3. A mass-delete circuit breaker (`--yes-delete-many`). Mirrors
   scripts/audit/audit_blocklist.py's `--yes-create-status-option`: when the script
...
from channel_vetting.airtable.client import _base_url, _headers, delete_record
...
from channel_vetting.enrichment.external_dedupe import fetch_external_handles
