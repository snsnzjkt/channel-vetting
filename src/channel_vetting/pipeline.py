...
from channel_vetting import config as _config
from channel_vetting.verification import video_topics
...
from channel_vetting.discovery.youtube_search import run_discovery
...
from channel_vetting.ranking.scoring import (
    calc_fake_follower_risk,
    calc_overall_score,
    QUALIFIED,
    qualify,
)
...
from channel_vetting.enrichment.external_dedupe import (
    fetch_external_handles,
    ExternalIndex,
    match_external,
)
from channel_vetting.discovery import rejected_handles
from channel_vetting.core.prospect_day import today_iso
from channel_vetting.budget.quota_tracker import can_afford_enrichment, get_today_spend
...
from channel_vetting.verification.gemini import GeminiVerifier
...
# and wire_discovery_filters() moved to discovery/niches.py (2026-08-14). They had to go
...
from channel_vetting.discovery.niches import (  # noqa: E402
    BIO_OFF_SIGNALS,
...
# NICHES lives in discovery/niches.py (extracted 2026-08-14) so outreach/sender.py can read the
# niche -> table mapping without importing this module and, with it, Playwright.
# Re-exported because this module and several tests refer to `pipeline.NICHES`.
from channel_vetting.discovery.niches import NICHES  # noqa: E402
...
#     calc_overall_score() at WEIGHT_NICHE_MATCH = 0.10 (ranking/scoring.py), so
...
# One label per step of the chain below. scripts/backfill/backfill_missing_emails.py
...
# Qualification value no longer exists (see ranking/scoring.py). Two more hard
...
# changed on 2026-08-20. It used to be that pipeline.resolve_country() read the
...
# scripts/audit/audit_prospects.py and look at where the reviewers' own Approved/Rejected
...
#   - Against ALREADY-TRACKED rows it is nearly irrelevant. scripts/audit/audit_prospects.py on
...
# scripts/audit/audit_prospects.py and look at where the reviewers' own Approved/Rejected calls
# actually fall.
...
# widened past television on 2026-08-25 (see discovery/niches.py).
...
# wrong. See verification/video_topics.py.
...
# see the comment on BROADCAST_TV_NAME_TERMS in discovery/niches.py for the measurement
...
def off_target_reason(niche_config: dict, description: str, video_titles) -> tuple[str | None, str]:
    ...
    REJECTED on 2026-08-15 (the reasoning is preserved in discovery/niches.py above
    ...
    # `off_target_categories` in discovery/niches.py. Absent the key, every category
...
def location_drop_reason(
    ...
    There is deliberately no browser step. See enrichment/email_browser.py: the About
...
def resolve_email_with_source(
    ...
    rather than a worse one. enrichment/email_influencers.py's module docstring carries that
    ...
    browser could never add an address. See enrichment/email_browser.py.
...
# csv_safe() and its inverse live in core/text_safety.py (extracted 2026-08-14) so
# outreach/sender.py can reuse them without importing this module — `import main`
...
# and several tests refer to `pipeline.csv_safe`.
# Re-exported ONLY for existing callers: this module still calls csv_safe(),
# scripts/backfill/backfill_missing_emails.py imports it from here, and tests/test_csv_injection.py
...
from channel_vetting.core.text_safety import (  # noqa: E402,F401
...
def process_candidate(
    ...
    # A candidate carries EITHER a "channel_id" (discovery/youtube_search.py / YouTube search)
    # OR a "handle" (discovery/influencers_club.py, which surfaces creators by
    ...
    # leads/influencer tables (see enrichment/external_dedupe.py) — checked here rather
    ...
    # broadcaster — see broadcast_tv_reason and the discovery/niches.py comment. Free,
    ...
    # pre_push_drop_reason's None rule. Shared with scripts/audit/audit_prospects.py; the
    ...
    # of 81 Approved and 5 of 130 Rejected channels (scripts/analysis/measure_video_topics.py).
    ...
    record = {
        ...
        # airtable.client.channel_exists() looks up by, so a leading
        ...
        # Attacker-influenced: chain step 5 (enrichment/email_browser.py) reads
        ...
        # (ranking/scoring.py / config.py), not third-party text.
    ...
    # verification.gemini._notes, because csv_safe only inspects value[0] and an
...
def _discovery_exclude_handles(
    ...
    on DO NOT CONTACT). See discovery/rejected_handles.py, and note the vendor's 10,000
...
def run_niche(
    ...
    if use_discovery:
        ...
        # "Handle" column exists (see airtable.client.get_tracked_handles), in
...
def run(
    ...
    # influencer tables (see enrichment/external_dedupe.py) — cached, so this is
    ...
    # no address was free (see enrichment/email_influencers.py). Reported because a credit
    ...
    if total_processed == 0 and total_discovered > 0:
        logger.error(
            ...
            "discovery — run `scripts/analysis/measure_discovery_pool.py --net` for the real "
