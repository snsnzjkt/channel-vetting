...
Imports NOTHING from this project (same rule as `text_safety.py`) so any
module can use it without a dependency cycle — `outreach_ledger.py` is
deliberately storage-agnostic and testable without Airtable, so it must not
have to import `enrichment.py` (and its `http_client`/`config` chain) just to
read a timestamp.
...
2. `outreach_ledger._parse_utc` returned None for the same shapes — and its
