...
audio (`verification.gemini.build_video_request` sends the video URL, not a
...
(see `pipeline.off_target_reason` and `config.GEMINI_TEXT_TIER`). This reports
...
def category_distribution(category_ids) -> list:
    ...
    enough that the measurement in `scripts/analysis/measure_video_topics.py` is the only thing
