...
result is real. `discovery/search_zones.py` falls back to the content-language
...
# step-4 one (see pipeline.EMAIL_SOURCE_* and scripts/backfill/backfill_missing_emails.py).
...
class BrowserEmailScraper:
    ...
    def find_contact(self, channel_id: str, need_email: bool = True) -> tuple[str, bool | None]:
        ...
        presence (pipeline.DROP_NO_SOCIAL). None (not False) on an unreadable list
