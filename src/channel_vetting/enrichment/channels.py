...
`core/http_client.py` (imported as HTTP), which carries the API key as a header
...
caller — pipeline.process_candidate() — handles a falsy return by skipping the
...
# per-video floor (pipeline.MIN_VIEWS_PER_VIDEO) only judges videos public at
...
# (see below) — still used for reporting in scripts/backfill/backfill_missing_emails.py.
FREEMAIL_DOMAINS = {
...
def count_longform(durations) -> int:
    ...
    feeds a MINIMUM (pipeline.MIN_LONGFORM_VIDEO_COUNT), so counting an unknown
...
def _parse_iso_timestamp(value: str | None) -> datetime | None:
    ...
    imports nothing — because `outreach/ledger.py` needed the identical rule and
...
def get_channel_stats(channel_id: str | None = None, *, handle: str | None = None) -> dict | None:
    ...
    The handle path exists for discovery/influencers_club.py, which surfaces
    ...
    # in core/http_client.py. It must not be reintroduced — `requests` embeds the
    ...
    try:
        ...
        # core/http_client.py), so this is a genuinely unreachable API rather than
...
def get_recent_video_performance(
    ...
    # every upload. See pipeline.off_target_reason.
    video_titles = []
    ...
    # with an empty body (measured 2026-08-24). See verification/video_topics.py.
    video_tags = []
    video_category_ids = []
    durations = []
    ...
    return {
        ...
        # had an unreadable duration. pipeline.pre_push_drop_reason discards
        ...
        # that window actually held. pipeline.pre_push_drop_reason needs BOTH:
        ...
        # above) and read by pipeline.off_target_reason to judge what the channel
...
def scan_older_videos_for_email(
    ...
    Step 3 of the email chain (see pipeline.resolve_email). Only worth running
...
def calc_uploads_per_year(upload_dates: list[str]) -> float | None:
    ...
    pipeline.py would shadow both.

    The single home of the "videos/month x 12" conversion, shared by pipeline.py
    and scripts/audit/audit_prospects.py so the pipeline and the audit script can never
