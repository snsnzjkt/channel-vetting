...
`country living home` ran 1 of 9. `pipeline.push_record` already writes the finding
...
def source_keywords(source: str) -> list:
    ...
    `pipeline.push_record` writes `"{SOURCE_LABEL} (kw1, kw2)"`, so the provenance is
    ...
    The vendor discovery path does not write a bare constant. `pipeline.run_niche`
    ...
    `pipeline.py`: the data was already there, and the fix belonged in the reader, not
    in the writer.
