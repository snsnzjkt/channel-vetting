...
| `INFLUENCERS_MAX_DISCOVERY_HANDLES_PER_DAY` | _(derived)_ | Caps handles bought in a **single day**, so the period allowance is paced rather than front-loaded. Empty derives it as period cap ÷ window — **127/day** now, **145/day** from 2026-10-12 — so it follows the introductory cap automatically. Floor division, so a full window of daily caps can never exceed the period cap. Set explicitly to spend harder (the pipeline runs weekdays only, so ~22 × 127 leaves ~1,170 handles unused); `0` stops paid discovery |
| `INFLUENCERS_HANDLE_INTRO_CAP` | 3965 | A **lower, temporary** handle cap that overrides the one above until `INFLUENCERS_HANDLE_INTRO_UNTIL`, set because the current vendor period is already partly spent. It lifts on its own — no code change and no deploy a month from now |
| `INFLUENCERS_HANDLE_INTRO_UNTIL` | 2026-10-12 | The date the introductory cap expires and the steady-state 4500 takes over. **Exclusive** — on this date the cap is already 4500. Set to `""` to retire the introductory cap early; an unparseable date falls back to the *lower* of the two caps, since an unreadable spend guard must not authorise more spend |
