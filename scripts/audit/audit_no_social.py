...
WHY THIS IS SEPARATE FROM scripts/audit/audit_prospects.py. That script re-checks every
...
`pipeline.DROP_NO_SOCIAL` fires on an EMPTY About link list, and the link list only
exists in the rendered page (`channels.list` does not expose links at all, which
is the whole reason enrichment/email_browser.py exists). So a row can pass the full audit
...
reading a page that is already public. Slow (a few seconds a row), not expensive.
    ...
    python scripts/audit/audit_no_social.py                         # report only
    python scripts/audit/audit_no_social.py --confirm --only "Name"  # delete, allowlisted
"""
import argparse
import logging
import time

from channel_vetting.airtable.client import delete_record, get_records
...
from channel_vetting.pipeline import DROP_NO_SOCIAL, NICHES

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
logger = logging.getLogger(__name__)

# Same contract as scripts/audit/audit_prospects.py: only rows no human has ruled on may be
...
# The mass-delete circuit breaker, matching scripts/backfill/cleanup_external_duplicates.py.
