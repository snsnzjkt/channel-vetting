...
@handles, not channel IDs (see influencer_discovery.py). main.py started
storing the handle on every new row in 2026-08, but the rows already in the
tables have an empty cell — so `airtable_client.get_tracked_handles()` returns
...
what get_tracked_handles() already tolerates.
    ...
    python backfill_handles.py            # report only
    python backfill_handles.py --confirm  # actually write
"""
import argparse
import logging
import time
...
from airtable_client import get_records, push_record, table_has_field
from config import API_SLEEP_SECONDS, YOUTUBE_API_BASE_URL
...
def resolve_handles(channel_ids: list[str]) -> dict[str, str]:
    ...
    for start in range(0, len(channel_ids), CHANNELS_LIST_BATCH):
        ...
        try:
            ...
            # Retries are already exhausted by here (see http_client.py), so
