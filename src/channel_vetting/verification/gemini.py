...
WHAT THIS IS FOR. `pipeline.off_target_reason` drops a candidate whose recent video
...
WHY RAW REST AND NOT `google-genai`. The same reason core/http_client.py gives for
...
from channel_vetting.core.http_client import GEMINI as HTTP, safe_body
...
def build_metadata_screen_request(niche_label: str, niche_description: str,
    ...
    REJECTED in this repo on 2026-08-15. `pipeline.off_target_reason` records exactly
    ...
    evidence. See scripts/analysis/measure_metadata_screen.py.
...
def verdict_confirms(payload: dict, min_confidence: float,
    ...
    # eligible. So a criterion marked required in discovery/niches.py disqualifies outright.
...
class GeminiVerifier:
    ...
    **Instance state, not module state**, and enrichment/email_influencers.py:61 states the rule
    ...
    Built once in `pipeline.run()` beside the InfluencersClient and threaded down to
    ...
    def from_config(cls):
        ...
        """
        from channel_vetting import config as cfg
        ...
        if not cfg.GEMINI_API_KEY:
            ...
            # pipeline.py already applies to USE_PLAYWRIGHT_STEALTH.
            logger.warning(
    ...
    def _load_cache(self) -> dict:
        ...
        optimisation, exactly as discovery/rejected_handles.py says of its own file. Losing
        ...
        if verdict.reason_code == QUOTA_EXHAUSTED:
            ...
            if self.reasons["consecutive_400"] >= 3:
                ...
                logger.error(
                    ...
                    "is OFF for this run. Re-run scripts/verify_video.py against the "
    ...
    def confirm_topic(self, topic: str, terms, performance) -> "TopicVerdict":
        ...
        of the tokens and none of the video ceiling. See verification/transcripts.py for the
