...
from http_client.py), not `requests.get` — `discovery` no longer calls
...
def test_non_200_does_not_populate_the_cache(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(discovery.HTTP, "get", lambda *a, **k: _Resp(503))
...
def test_clean_completion_still_caches_normally(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(discovery, "record_spend", lambda *a, **k: None)
    monkeypatch.setattr(discovery, "can_afford_search", lambda: True)
    monkeypatch.setattr(discovery.HTTP, "get", lambda *a, **k: _Resp(200, _search_payload(["UC1"])))
...
def test_network_exception_does_not_raise_and_does_not_cache(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(discovery.HTTP, "get", boom)

    result = discovery.discover_channels_by_keyword("kw", max_results=50)

    assert result == []
    assert discovery._cache_key("kw") not in discovery._load_cache()
...
def test_healthy_call_after_a_network_exception_re_queries(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(discovery.HTTP, "get", flaky)

    assert discovery.discover_channels_by_keyword("kw", max_results=50) == []
    second = discovery.discover_channels_by_keyword("kw", max_results=50)
    assert {c["channel_id"] for c in second} == {"UC1", "UC2"}
...
def test_record_spend_not_called_on_non_200(monkeypatch, tmp_path):
    ...
    spends = []
    monkeypatch.setattr(discovery, "record_spend", lambda units, **k: spends.append(units))
    monkeypatch.setattr(discovery.HTTP, "get", lambda *a, **k: _Resp(403))

    discovery.discover_channels_by_keyword("kw", max_results=50)
...
def test_record_spend_is_called_on_a_served_200(monkeypatch, tmp_path):
    ...
    assert spends == [discovery.QUOTA_COST_SEARCH_LIST]
...
def test_params_never_carry_the_api_key(monkeypatch, tmp_path):
    ...
    Actions log kept for 90 days. See http_client.py's module docstring."""
    import discovery
    ...
    monkeypatch.setattr(discovery.HTTP, "get", capture)

    discovery.discover_channels_by_keyword("kw", max_results=50, days_back=7)
...
def test_save_cache_is_atomic_and_leaves_no_tmp_file(monkeypatch, tmp_path):
    ...
    discovery._save_cache(original)
    ...
    with monkeypatch.context() as m:
        m.setattr(discovery.json, "dump", exploding_dump)
        with pytest.raises(OSError):
            discovery._save_cache({"2026-08-10::other": []})
        assert not list(tmp_path.glob("*.tmp")), "a stale .tmp must not be left behind"

    assert discovery._load_cache() == original
...
def test_today_pacific_is_public_and_the_private_alias_still_resolves():
    """discovery.py shares this exact helper so the cache day and the quota
