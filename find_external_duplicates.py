...
added externally that morning. Fixed at the call site in `main.run` — the index
is now rebuilt every run. This script cleans up what got through before that.

    python find_external_duplicates.py                 # report
    python find_external_duplicates.py --delete --yes  # act, unjudged only
"""
import argparse
import sys
...
def main(argv=None) -> int:
    ...
    if not args.delete:
        ...
        print("  python find_external_duplicates.py --delete --yes")
        if judged:
