...
# the split has to happen at the call site: enrichment/email_influencers.py handles both and
...
# enrichment/email_influencers.py — where the wait IS capped (MAX_RATE_LIMIT_WAIT_SECONDS)
...
# enrichment/email_influencers.py as a RequestException, which returns "" and counts toward
...
# discovery/youtube_search.py raises its own clear "YOUTUBE_API_KEY is not set" error, and
...
def _retry_after_seconds(resp: requests.Response, default: float) -> float:
    ...
    # run — the same trap enrichment.email_influencers._wait_seconds() guards against.
