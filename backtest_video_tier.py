...
`backtest_relevance.py` cannot do it. That script reads `gemini_cache.json` and
...
ratio, per GEMINI_VERIFY_PLAN.md).
    ...
    python backtest_video_tier.py
    python backtest_video_tier.py --min-rows 30   # refuse to report below n
"""
import argparse
import itertools
import random
...
def parse_detail(detail: str) -> dict:
    ...
    The field is prose assembled by `gemini_verify.judge`, so this reads it
