...
signal could not ship, and 14.16 records the AI screen that failed to.
    ...
    python scripts/rank_pending.py                 # both niches
    python scripts/rank_pending.py --niche "Home Theater"
    python scripts/rank_pending.py --show-rates    # the keyword table it learned
"""
import argparse
...
def main(argv=None) -> int:
    ...
    for niche, table in NICHE_TABLES:
        ...
        rates = keyword_history.approval_rates(judged)
        ...
        if args.show_rates:
            ...
            print(f"\n  keyword table ({len(usable)} usable, {thin} too thin at "
                  f"<{keyword_history.MIN_LABELLED_FOR_RATE} verdicts):")
        ...
        ordered = keyword_history.rank(pending, rates)
        ...
        scored = [r for r in ordered if r["priority"] != keyword_history.NEUTRAL_PRIOR]
