...
def today_pacific() -> str:
    ...
    Public because `discovery/youtube_search.py` keys its search cache on the same day
...
def _save_log(log: dict) -> None:
    ...
    airtable.client.count_added_today() was deliberately hardened against by
    ...
    """
    log = _prune(log)
    # UNIQUE PER PROCESS. This was f"{QUOTA_LOG_FILE}.tmp" — one shared name —
    ...
    tmp_path = f"{QUOTA_LOG_FILE}.{os.getpid()}.tmp"
    try:
