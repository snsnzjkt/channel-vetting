...
Why this is NOT built on `airtable_client.push_record()`
--------------------------------------------------------
`push_record()` is hard-keyed on "Channel ID" and PATCHes the existing row when
it finds one (`airtable_client.py:266-271`). Pointed at the ledger it would
...
`airtable_client.count_added_today()` raises: failing open is the one direction
...
from airtable_client import _base_url, _headers, _quote_formula_value
from config import (
...
from outreach_ledger import SENDABLE_QUALIFICATION, LedgerUnavailable
...
def mark_contacted(table_name: str, record_id: str) -> bool:
    ...
    belong together. It was `outreach.py` reaching for a private `_patch`
    ...
    `typecast=False` matters here specifically: `audit_blocklist.py` writes a
...
class AirtableLedgerStore:
    """`outreach_ledger.LedgerStore` against the global Outreach Log table."""
...
class AirtableLeaseStore:
    """
    `outreach_ledger.LeaseStore` against the single-row Outreach Lock table.
    ...
    `python outreach.py` overlapping with a scheduled one, which CI cannot see.
    """
