...
`airtable.client.push_record()`), so two concurrent runs interleave:
...
been accepted and the response lost. `enrichment/email_influencers.py` already reasons this way
...
`airtable/outreach_store.py`. Note that store MUST send `typecast=False` on the
...
# into the Airtable single-select, and pipeline.py already imports the constant
...
# hand-built filterByFormula. ranking/scoring.py imports only `math`, so no cycle.
SENDABLE_QUALIFICATION = QUALIFIED
REASON_NOT_QUALIFIED = "not_qualified"

# --- Demo mode lives in outreach/mailer.py --------------------------------------------
...
class LedgerUnavailable(RuntimeError):
    ...
    for the same reason `airtable.client.AirtableReadError` does: a silent
...
class LedgerStore(Protocol):
    """
    The storage seam. Implemented against Airtable in `airtable/outreach_store.py`
...
class RunBudget:
    ...
    reason `airtable.client.count_added_today()` reads Airtable: a per-RUN cap
