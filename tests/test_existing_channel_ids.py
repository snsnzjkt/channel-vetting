...
shared session now retries 429s: these tests mock at `airtable.client.HTTP`
...
def test_collects_ids_across_pages(monkeypatch):
    ...
    assert client.get_existing_channel_ids("tblFake") == {"UC1", "UC2"}
...
def test_raises_on_non_200_partway_through_pagination(monkeypatch):
    ...
    with pytest.raises(client.AirtableReadError):
        client.get_existing_channel_ids("tblFake")
