...
Unlike airtable/do_not_contact.py, read failures here LOG AND RETURN PARTIAL
...
EXTERNAL_HANDLES_CACHE_FILE = data_path("external_handles_cache.json")
...
class ExternalIndex:
    ...
    # (scripts/backfill/cleanup_external_duplicates.py's `in`/`[]`/`len`, and the discovery
...
def _fetch_table_entries(table_id: str, link_field: str, name_field: str) -> tuple[set[str], set[str]]:
    ...
    while True:
        ...
        try:
            ...
            # airtable/do_not_contact.py, which must fail closed): a missing entry
