...
Run `python outreach.py` for a dry run (the default). `--send` is required to
actually deliver, and even then `OUTREACH_DEMO_MODE` redirects every message to
...
Exit codes are a contract (`main.py` set the precedent: a scheduled run that
...
import outreach_templates as templates
from config import (
...
from enrichment import EMAIL_PATTERN
from mailer import DemoModeError, MailerError, build_message, from_config, recipient_for
from niches import NICHES
from outreach_airtable import (
    AirtableLedgerStore,
    AirtableLeaseStore,
    get_queued_prospects,
    mark_contacted,
)
from outreach_ledger import (
    LedgerUnavailable,
    acquire_lease,
    release_lease,
    RunBudget,
...
PREVIEW_DIR = Path("outreach_preview")
    ...
    for niche in NICHES:
        if niche not in templates.TEMPLATES:
            problems.append(
                ...
                f"outreach_templates.TEMPLATES or the niche cannot be contacted"
            )
    if send_mode:
...
def write_preview(campaign: str, channel_id: str, rendered: dict, *, to: str, sender: str) -> Path:
    ...
    answer it, and the repo's own precedent (cleanup_external_duplicates.py)
    ...
    def print(self, *, campaign: str, mode: str, remaining: int, stranded: int) -> None:
        ...
        if stranded:
            logger.warning(
                ...
                "python outreach.py --reconcile", stranded,
            )
...
def run(args) -> int:
    ...
    if args.reconcile:
        ...
        for row in stranded:
            f = row.get("fields", {})
            logger.warning(
                "  %s  claimed %s  ->  python outreach.py --settle %s --state sent|notsent",
    ...
    # hand-run `python outreach.py` overlapping with the scheduled CI job — the
    # GitHub Actions `concurrency: outreach` group serialises CI against CI and
    ...
    if queue_was_empty:
        ...
        # train people to ignore the X. main.py's non-zero-on-nothing is
