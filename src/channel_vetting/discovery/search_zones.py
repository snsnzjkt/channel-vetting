...
# to a niche is `ZONE_CORE | EUROPE_COUNTRY_CODES` in discovery/niches.py rather than a
...
# and `REQUIRED_NICHE_KEYS` in pipeline.py makes that key mandatory precisely so a
...
def zone_verdict(raw: str | None, allowed_codes=ALLOWED_COUNTRY_CODES) -> bool | None:
    ...
    **A None verdict is now a DROP, not a keep** (changed 2026-08-20). What
    ...
    `pipeline.process_candidate`, which used to discard only on `is False` and
