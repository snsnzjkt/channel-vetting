...
writes to one table per niche (see NICHES in pipeline.py).
"""
import logging
import time
...
from channel_vetting.config import AIRTABLE_TOKEN, AIRTABLE_BASE_ID, API_SLEEP_SECONDS
...
# retry adapter mounted on the shared session (see core/http_client.py). Pacing
...
def _quote_formula_value(value: str) -> str:
    ...
    path is safe. But `scripts/backfill/backfill_missing_emails.py` feeds `channel_exists()`
...
def get_records(table_name: str, fields: list[str] | None = None) -> list[dict]:
    ...
    a specific row (scripts/audit/audit_prospects.py). Pass `fields` to limit what comes back
...
# are only ever meant to be set by pipeline.py at CREATE time (fresh
# defaults) or by a caller that deliberately opts in via
# `overwrite_status_and_notes=True` (e.g. scripts/audit/audit_blocklist.py's --mark,
...
def push_record(table_name: str, record: dict, overwrite_status_and_notes: bool = False) -> bool:
    ...
    try:
        if existing_record_id:
            ...
            # rejected the request WITHOUT processing it. See core/http_client.py.
